<template>
  <el-row class="messageView" v-loading="loading">
    ...
    <el-col :span="11">
      ...
      <el-row>滚动时间：{{ afterContent | playTimeText }}</el-row>
      <el-row
        >滚动次数：{{
          afterContent.noLimit === 1 ? '不限次数' : afterContent.scrollTimes
        }}</el-row
      >
      <el-row
        >滚动内容：<span style="word-break: break-all">{{
          afterContent.messageText
        }}</span></el-row
      >
    </el-col>
    <el-col :span="12" class="right">
      ...
      <el-row>滚动时间：{{ frontContent | playTimeText }}</el-row>
      <el-row>滚动次数：{{ frontContent.scrollTimes }}</el-row>
      <el-row>
        滚动内容：<span>{{ frontContent.messageText }}</span>
      </el-row>
    </el-col>
  </el-row>
</template>
