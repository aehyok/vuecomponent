<template>
  <div>
    <el-dialog
      title="审核历史"
      :visible="dialogTableVisibleChild"
      @open="handOpened"
      @close="$emit('update:dialogTableVisible', false)"
    >
      <el-table :data="gridData" border v-loading="isLoading">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              type="text"
              @click="getAuditDetail(scope.row)"
              v-if="scope.row.frontContent"
              >变更详情</el-button
            >
            <span v-else>--</span>
          </template>
...
export default {
  ...
  methods: {
    getAuditDetail(row) {
      console.log(row)
      if (row.frontContent.contentType !== 7) {
        const { href } = this.$router.resolve({
          path: `/column-content-history?id=${row.id}&auditor=${row.auditor}&updatedAt=${row.updatedAt}`,
        })
        window.open(href, '_blank')
      } else {
        const { href } = this.$router.resolve({
          path: `/investigate-history?id=${row.id}&auditor=${row.auditor}&updatedAt=${row.updatedAt}`,
        })
        window.open(href, '_blank')
      }
    },
