...
export default {
  ...
  mounted() {
    ...
    xhr.open('GET', 'http://192.168.88.158/hhl/980031.ts', true)
    // 接收的是 video/mp2t 二进制数据，Blob类型也可以，但arraybuffer类型方便后续直接处理
    xhr.responseType = 'arraybuffer'
    xhr.send()
    console.log(xhr.readyState, 'xhr----ssss')
    xhr.onload = function() {
      console.log(xhr, 'xhr1----ssss')
      if (xhr.readyState === 4) {
        console.log(xhr, 'xhr2----ssss')
        if (xhr.status === 200) {
          console.log(xhr.response, 'ssss')
  ...
  methods: {
    ...
    transferFormat(data) {
      ...
      transmuxer.on('done', function() {
        ...
        this.vjsParsed = muxjs.mp4.tools.inspect(bytes)
        console.log('transmuxed', this.vjsParsed)
    ...
    prepareSourceBuffer(combined, outputType, bytes) {
      ...
      this.$('#video-wrapper').appendChild(this.video) // 将H5 video元素添加到对应DOM节点下
      ...
      this.mediaSource.addEventListener('sourceopen', function() {
        ...
        // 转换为带音频、视频的mp4
        if (combined) {
          buffer = this.mediaSource.addSourceBuffer(
        ...
        this.video.addEventListener('error', this.logevent)
