<template>
  <div>
    <TablePanel
      :data="list"
      :columns="columns"
      @selection-change="handleSelectionChange"
    />
  </div>
</template>

<script>
import TablePanel from '@/components/TablePanel'
import { format } from 'date-fns'
import { auditStatusTxt } from '@/utils/currency'
export default {
  components: {
    TablePanel,
  },
  props: {
    list: {
      type: Array,
      default: () => [],
    },
  },
  data() {
    return {
      columns: [
        // {
        //   type: 'selection',
        // },
        {
          label: '栏目树名称',
          prop: 'name',
        },
        {
          label: '提交人',
          prop: 'submitter',
        },
        {
          label: '提交审核时间',
          prop: 'submitDateTime',
          render: (h, { row }) => {
            return format(new Date(row.submitDateTime), 'yyyy-MM-dd HH:mm:ss')
          },
        },
        {
          label: '审核状态',
          prop: 'auditStatus',
          render: (h, { row }) => {
            return (
              <el-tag type={auditStatusTxt[row.auditStatus].type}>
                {auditStatusTxt[row.auditStatus].text}
              </el-tag>
            )
          },
        },
        {
          label: '操作',
          width: 240,
          render: (h, { row }) => {
            return (
              <div>
                <el-button type="text" onClick={() => this.examine(row)}>
                  审核
                </el-button>
              </div>
            )
          },
        },
      ],
    }
  },
  methods: {
    handleSelectionChange(selection) {
      console.log(selection)
    },
    examine(row) {
      this.$emit('saveFilterData')
      this.$router.push(
        `/columntreereview/detail?recordId=${row.id}&name=${row.name}&orgId=${row.id}&submitter=${row.submitter}&aduitType=audit`,
      )
      console.log(row)
    },
  },
}
</script>
