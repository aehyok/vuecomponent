<template>
  ...
  >
    ...
    >
      ...
      <el-form-item class="formItem" label="霸屏内容类型：" prop="type">
      ...
      >
      <el-checkbox-group v-model="formData.images"></el-checkbox-group>
        <PictureGroup
          :picType="thisType"
          :limit-count="20"
          @add="handleImageAdd"
      ...
      >
        <el-checkbox-group v-model="formData.videos"></el-checkbox-group>
        <VideoPicker
          :thisType="thisType"
...
<script>
import { createSuperMessage, auditSuperMessage } from '@/services/message'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rangeVisible: false,
      popupTypes: [{ label: '图片', value: 1 },{label: '视频', value:2 }],
      thisType: 'add',
      serviceProvider: {
        default: 0,
      },
      ...
      rules: {
        ...
        images: [{ required: true, message: '请选择霸屏图片' }],
        videos: [{required: true, message: '请选择霸屏视频'}],
  ...
  computed: {
    dialogTitle() {
      return this.source.id ? '修改霸屏消息' : '创建霸屏消息'
    },
    imageId() {
  ...
  methods: {
    ...
    handleImageRemove({ id }){
      const idx = this.formData.images.findIndex(i => i.id === id)
    ...
    save(audit) {
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          const res = await createSuperMessage({
            ...this.formData,
            startDate: startDate.getTime(),
            endDate: endDate.getTime(),
            images: this.formData.images.map(item => item.id ),
            videos: this.formData.videos.map(item => item.id ),
            id: this.source.id,
          })

          if (res.code === 200) {
            if (audit) {
              const res2 = await auditSuperMessage({
